...
A13, Deployment:
With deployment come a few changes. Instead of using a single settings file, we create one for our development and production environments. The development
sets debug to true and sets allowed hosts to the localhost. The production sets debug to false and also includes the public url in allowed hosts. These bots inherit from
a base settings file which makes them consistent. We also generate a requirements.txt which allows for the rapid creation of virtual environments by providing the needed libraries.
A few other changes include moving the db to a data file and some other minor settings to accommodate our changes.

At deployment, we use python anywhere. We clone our repository and set up the virtual environment. After that, we go through the settings and wire everything up. Unfortunately, my project is 
designed with always-on tasks, and pythonanywhere does not allow that for free users. I my site is accessible though pythonanywhere but the actual functionality of the dashboard is limited. Lastly,
I noticed it is very slow. Trying to access a chart generated by matplotlib causes the site to lock up.